...
xyz = 'word.csv'
filename = os.path.join('csvs', xyz)
print(filename)

def pie(filename=filename):
    df = pd.read_csv(filename)
    plt.figure(figsize=(4,4))
    recipe = list( df.groupby('Shift').count()['Time'].index )
    data = list(df.groupby('Shift').count()['Time'].values)
    label = [str(recipe[i] + '\n'+str(data[i])+' msgs') for i in range(2) if len(recipe) >= 2 and len(data) >= 2]

    fig = px.pie(values=data, names=label, title='Messages in respective Meridian')
    fig.write_html(os.path.join('static/',xyz+'pie.html'))
'''    plt.pie(data, textprops=dict( fontsize=15,
        color="black"), wedgeprops=dict(width=0.45), startangle=20 ,labels=label)
...
def word(filename=filename):
    ...
    wordcloud = WordCloud(width = 700, height = 500,
                    ...
                    min_font_size = 10,
                    max_font_size = 150,
                    colormap= None).generate(comment_words)

    plt.title("WORD CLOUD",fontsize=40)
    plt.figure(1)
    plt.imshow(wordcloud) 
    plt.axis("off") 
    plt.savefig(os.path.join('static/images/dashboard',xyz+'word.svg') ,bbox_inches='tight')

# return the number of messages in the csv file
def number_of_msgs(filename=filename):
    df = pd.read_csv(filename)
    return df.shape[0]
...
def most(filename):
    ...
    # fig= px.bar(df, x='Contacts', y='Time')
    # fig.update_layout(xaxis_title='Messages', yaxis_title='Users')
    # fig.write_html(os.path.join('static/',xyz+'mactive.html'))
    df = pd.read_csv(filename)
    plt.figure(figsize=(8, 6))
    ...
    try:
        ...
        print("The 'Contacts' column is not found in the DataFrame.")
...
def least(filename):
    ...
    fig.write_html(os.path.join('static/',xyz+'active.html'))
    ...
    fig.write_html(os.path.join('static/',xyz+'mwise.html'))
    ...
    fig.write_html(os.path.join('static/',xyz+'sentipie.html'))
...
# Lemmatization - From plural to single + Base form of a word (example better-> good)
    ...
    fig.update_layout(xaxis_title='Month', yaxis_title='Message Count')
    fig.write_html(os.path.join('static/',xyz+'sentibar.html'))
...
def week(filename):
    ...
    fig.update_layout(xaxis_title='Month', yaxis_title='Message Count')
    fig.write_html(os.path.join('static/',xyz+'week.html'))
